...
> **QUY CHUẨN VẼ HÌNH TIKZ & BÀI TẬP ĐIỀN KHUYẾT (Thầy chốt 09/09/2026):**
> 1. **Vị trí nhãn tâm $O$ tại giao điểm 2 đường chéo:** Tuyệt đối **CẤM đặt `below right`** nếu có đường chéo chạy từ trên-trái xuống dưới-phải (đường chéo sẽ chèn xuyên qua chữ $O$). Bắt buộc đặt `node[below=3pt]{$O$}` hoặc `node[left=3pt]{$O$}` vào vùng tam giác trống giữa các đường chéo.
> 2. **Ký hiệu góc vuông tại giao điểm đường chéo nghiêng $45^\circ$:** Tuyệt đối **CẤM dùng `-|`** (vẽ ngang/dọc làm góc vuông bị xoay lệch thành hình mũi tên). Bắt buộc dùng tọa độ xoay $45^\circ$ ôm theo hai đường chéo: `\draw[ink] (x0-d, y0+d) -- (x0, y0+2d) -- (x0+d, y0+d);`.
> 3. **Tọa độ hình học phải khớp giải tích chính xác:** Nếu điểm $D$ thuộc cạnh huyền $BC$ của tam giác vuông $ABC$, tọa độ $D$ phải thỏa mãn phương trình đường thẳng $BC$, tuyệt đối không vẽ ước lượng làm đường thẳng không chạm đỉnh ("k chạm đỉnh $D$").
> 4. **Không vẽ ký hiệu góc vuông bị đường khác cắt qua:** Không vẽ ô vuông góc vuông tại đỉnh xuất phát của đường chéo hoặc đoạn thẳng phân giác (bị đường nối cắt đôi trông lem nhem).
> 5. **BẮT BUỘC có BÀI GIẢI ĐIỀN KHUYẾT (Scaffolded Proof Completion) trong phần Luyện tập:** Tại Chặng 3 hoặc Chặng 4 (Thông hiểu & Vận dụng), phải có bài tập điền vào chỗ trống (`[[blank:...]]`) cho các câu lập luận chứng minh hình học để HS nắm cấu trúc, mẫu trình bày và các bước suy luận trước khi tự làm bài tự luận độc lập.
> 6. **QUY TRÌNH XỬ LÝ SLIDE TRÌNH CHIẾU & CHUẨN FOOTER (Thầy chốt 09/09/2026):**
>    - **Triết lý:** Slide Beamer (16:9) là công cụ để Thầy tương tác và viết bảng, không phải sách giải. Tuyệt đối không chiếu sẵn lời giải làm lộ đáp án và làm học sinh mất tập trung.
>    - **Ví dụ mẫu:** Tự động cắt bỏ phần Lời giải (`strip_example_solution`), chỉ chiếu đề bài + hình vẽ TikZ. Nếu cần tùy biến, khai `text_slide` trong `NotedBlock`.
...
> **VÍ DỤ MẪU NẰM TRONG PHẦN LUYỆN TẬP (SONG SONG VỚI BÀI TẬP) CHO PHIẾU B+C (Thầy chốt 08/09/2026):**
> 1. **Không dồn các khối Ví dụ mẫu (`variant: "example"`) thành một chuỗi ở Chặng 2 (Kiến thức cần nhớ).** Chặng 2 chỉ giữ lại định nghĩa, công thức cốt lõi, bảng tóm tắt, bẫy điểm (`callouttrap`), đích thi vào 10 (`callouttarget`).
> 2. **Chuyển các khối Ví dụ mẫu xuống Chặng 3 (`practice1`) hoặc Chặng 4 (`practice2`), đặt ngay trước nhóm bài tập của dạng bài đó.**
> 3. **Luồng sư phạm song song trên lớp:** GV giảng và phân tích bài giải mẫu trong khung `VÍ DỤ MẪU` $\to$ HS áp dụng làm ngay các bài tập tương tự của dạng đó $\to$ tiếp tục sang dạng bài kế tiếp.
...
> 6. **Tên PDF đầu ra TỰ NÓI RA MÌNH LÀ BÀI GÌ** (Thầy chốt 06/09/2026, code ở [src/exporters/ten_file.py](src/exporters/ten_file.py), 4 test): `Toan8B-Tuan10-On-tap-hinh-binh-hanh-hinh-chu-nhat-Phieu-HS.pdf` = `<khối><tầng>-Tuan<NN>-<chủ đề lấy từ slug phiếu>-<bản in>`, chữ KHÔNG DẤU như tên thư mục Drive. Ba bản in: `Phieu-HS` (handout) · `Dap-an-GV` (guide) · `Slide`. **Lý do:** trước đây mọi thư mục trong kho đều đẻ ra đúng ba cái tên `ca-01-handout.pdf` / `ca-01-guide.pdf` / `ca-01-slide.pdf`, nên gửi hàng loạt PDF vào Zalo thì người nhận không biết file nào là bài nào nếu không gửi kèm cả thư mục. Thiếu dữ kiện (seed nằm ngoài cây `tuanNN-…`) thì lùi về tên cũ `ca-NN-<bản>` chứ không đoán bừa; PDF cũ trong kho vẫn đọc được vì `drive_sync` và `staleness_gate` nhận cả hai kiểu tên.
...
> 3. **Khối 55% VD+VDC chia theo TẦN SUẤT VDC của chương**, không chia đều: `p ≥ 0,50` → VD 35 · VDC 20 · `0,20 ≤ p < 0,50` → 43/12 · `0 < p < 0,20` → 50/5 (không đủ một câu VDC 18′ ⇒ dồn VDC vào phiếu ôn tập chương) · `p = 0` → 55/0. Đo lại bằng `.venv/bin/python scripts/tan_suat_vdc.py`; số nằm ở `vdc.phan_bo_55` của bản đồ, `tier_spec.tier_ratio` tự áp. **Mẫu dưới 6 đề ⇒ `nhom_uu_tien: "mau-qua-nho"`, `phan_bo_55: null`, giữ 15-30-55 mặc định — thà không soi còn hơn soi theo 2 đề.**
>
> 4. **Gán chương theo CÁCH GIẢI, không theo bối cảnh đề bài** (Thầy chốt 04/09/2026: *"HS có học tam thức bậc 2 để xét min max hả? KHÔNG ĐƯỢC GIẢI THEO CÁCH XÉT ĐẠO HÀM"*). Bài cực trị lớp 9 giải bằng **hằng đẳng thức $(x-a)^2+b$** hoặc **Cô-si** ⇒ chương II, KHÔNG phải chương VI — soi 33 câu thì **chương VI được 0 câu VDC**. Bảng quy ước ở `quy_uoc.gan_chuong_theo_cach_giai`, từng câu ở [inputs/refs/de-thi/lop-9/vdc-phan-loai.json](inputs/refs/de-thi/lop-9/vdc-phan-loai.json).
>
> Số liệu chốt luật lớp 9 (21 đề GK1/CK1 đủ điểm + 14 đề GK2/CK2): **27/33 đề (82%) kết thúc bằng bài CỰC TRỊ/TỐI ƯU 0,5đ** — cách giải: Cô-si 2 số 10 · hằng đẳng thức 9 · BPT ràng buộc 5 · Cô-si 3 số 2 · BĐT có căn 1 · BĐT nâng cao 1; **34/34 ý cuối bài hình là câu CHỨNG MINH nhiều bước**, xếp hạng: thẳng hàng 8 · vuông góc 5 · song song–đồng dạng 4 · hệ thức 4.
> **CÔ-SI NGOÀI SGK lớp 9** (bảng tra cứu SGK tập 1 tr.118 chỉ có: BĐT, hai BĐT cùng/ngược chiều, tính chất bắc cầu, BPT bậc nhất) — Cô-si HAI số vẫn dạy được vì quy về $(a-b)^2\ge0$, nhưng **Cô-si BA số thì không**; 4/33 câu cần công cụ ngoài SGK.
> Khối 6/7/8 (bản 0.6, phân loại bằng mắt 123 câu): **lớp 6** chương II *tính chia hết* p=0,84 và chương VI *phân số* p=0,77 (khuôn: tổng luỹ thừa chia hết · tổng dãy sai phân); **lớp 7** chương I p=0,78, IX p=1,00, VII p=0,36; **lớp 8** chương II *hằng đẳng thức* p=0,67, III p=1,00 — **kỳ II lớp 8 chỉ có 2 đề đọc được nên chưa kết luận**.
